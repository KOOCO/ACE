...
#region 遊戲流程控制
    ...
    public IEnumerator IStartGameFlow(GameFlowEnum gameFlow)
    {
        ...
        UpdateGameRoomData(roomData, () =>
        {
            ...
            JSBridgeManager.Instance.UpdateDataFromFirebase($"{QueryRoomPath}/{FirebaseManager.BET_ACTION_DATA}",
                                                            betActionData, gameObject.name, nameof(GameFlow));
        });
    }
    private IEnumerator GameFlow()
    {
        ...
        GameFlowEnum gameFlow = preUpdateGameFlow;
        switch (gameFlow)
        {
            //發牌
...
#region Old not use
    ...
    private IEnumerator ILocalGameFlowBehavior()
    {
        ...
        switch ((GameFlowEnum)gameRoomData.currGameFlow)
        {
            ...
            case GameFlowEnum.Licensing:
                ...
                yield return new WaitUntil(() => isComplet);
                isComplet = false;
    ...
    private IEnumerator ReturnGameFlow()
    {
        yield return IStartGameFlow(GameFlowEnum.Licensing);
    }
    ...
    private IEnumerator IJudgeNextSeason()
    {
        ...
        if (gameRoomData.hostId == DataManager.UserId)
        {
            ...
            Debug.Log("GameControl :: IJudgeNextSeason : canActionPlayers : " + canActionPlayers.Count);
            Debug.Log("GameControl :: IJudgeNextSeason : allInPlayers : " + allInPlayers.Count);
            Debug.Log("GameControl :: IJudgeNextSeason : foldPlayers : " + foldPlayers.Count);
            Debug.Log("GameControl :: IJudgeNextSeason : playingPlayers : " + playingPlayers.Count);
            yield return new WaitForSeconds(1);
            ...
            Debug.Log("GameControl :: IJudgeNextSeason : isAllBet : " + isAllBet);
            ...
            Debug.Log("GameControl :: IJudgeNextSeason : isAllPlayerBetValueEqual : " + isAllPlayerBetValueEqual);
            ...
            Debug.Log("GameControl :: IJudgeNextSeason : isBetValueEqual : " + isBetValueEqual);

            //剩下一名玩家可行動，其他玩家棄牌/離開/PlayerFold
            if (foldPlayers.Count() == gameRoomData.playingPlayersIdList.Count() - 1)
            {
                Debug.Log("GameControl :: IJudgeNextSeason : OnePlayerLeftResult : PlayerFold");
            ...
            {
                Debug.Log("GameControl :: IJudgeNextSeason : AllIn/Fold");
            ...
            {
                Debug.Log("GameControl :: IJudgeNextSeason : PotResult 1");
                ...
                isAllPlayerBetValueEqual == true)
            {
                ...
                Debug.Log("GameControl :: IJudgeNextSeason : " + nextFlow);
...
#region 遊戲資料更新
    ...
    private void GameDataInit(UnityAction callback)
    {
        ...
        foreach (var id in gameRoomData.playerDataDic.Keys)
        {
            ...
            if ((newButtonSeat == oldButtonSeat && currRoundCount > 0) || (newButtonSeat != 0 && currRoundCount == 0))
                ...
                {
                    count++;
                    if (count == gameRoomData.playerDataDic.Count) callback.Invoke();
                });
            });
        }
    ...
    public void UpdataPlayerData(string id, Dictionary<string, object> dataDic, UnityAction callback = null)
    {
        if (callback == null)
        {
        ...
        {
            JSBridgeManager.Instance.UpdateDataFromFirebase($"{QueryRoomPath}/{FirebaseManager.PLAYER_DATA_LIST}/{id}", dataDic, null, null, () =>
            {
                callback.Invoke();
            });
        }
    ...
    public void UpdateGameRoomData(Dictionary<string, object> data, UnityAction callback = null)
    {
        if (callback == null)
        {
            JSBridgeManager.Instance.UpdateDataFromFirebase($"{QueryRoomPath}", data);
        }
        else
        {
            JSBridgeManager.Instance.UpdateDataFromFirebase($"{QueryRoomPath}", data, null, null, () =>
            {
                callback?.Invoke();
            });
        }
    ...
    private void UpdateNextPlayer()
    {
        ...
        Debug.Log("GameControl :: IJudgeNextSeason : UpdateNextPlayer : Next Player Id :" + nextPlayerId);
        Debug.Log("GameControl :: IJudgeNextSeason : UpdateNextPlayer : PlayersCount :" + players.Count);
        //相同的玩家(遊戲只剩1人進行)
    ...
    public void UpdateBetAction(string id, BetActingEnum betActing, double betValue)
    {
        ...
        UpdataPlayerData(id, playerData, () =>
        {
            //UpdataPlayerData(id, playerData, (json) =>
            //更新下注行為
            var betActionData = new Dictionary<string, object>()
            {
    ...
    public void UpdateCarryChipsCallback()
    {
